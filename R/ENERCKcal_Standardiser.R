...
#'   Alcohol in grams \code{'ALCg'} are combined to fins the Energy in kcal.
...
ENERCKcal_standardised <- function(PROTg, FATg_standardised, CHOAVLDFg, FIBTGg, ALCg){

  ALCg[is.na(ALCg)] <- 0
  FIBTGg[is.na(FIBTGg)] <- 0
  ENERCKcal_std <- PROTg*4 + FATg_standardised*9 + CHOAVLDFg*4 + FIBTGg*2 + ALCg*7
