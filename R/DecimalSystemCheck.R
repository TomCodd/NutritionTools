...
#' @param first Required - The first column of the decimal system - the most
#'   basic item ID; e.g. 01 .
#' @param second Required - The second column of the decimal system - the first
#'   subdivision from the base ID; e.g. 01.005 .
#' @param third Optional - The third column of the decimal system - the second
#'   subdivision from the base ID and the first from the second ID; e.g.
#'   01.005.03 .
#' @param fourth Optional - The fourth column of the decimal system - the third
...
#'
#' @return An R dataframe detailing the errors found for each item in the
#'   decimal system.
#'
#' @examples
...
Decimal_System_Checker <- function(df, first, second, third, fourth) {
  ...
  output_df <- data.frame(matrix(ncol = 5, nrow = nrow(df))) #creating a model (empty) output dataframe
  colnames(output_df) <- c("primary code", "secondary code", "tertiary code", "quaternary code", "error") #renaming columns in output data frame

  total_error_count <- 0 #creating an error count and setting it to zero

  for(i in 1:nrow(df)){ #looping through each row of the input dataframe

    row_error_count <- 0 #creates an error count for the row in question, sets it to 0
    row_error_code <- c() #creates an empty error list for the row in question

    primary_value <- as.character(df[[first]][i]) #finds the first value for that row
    secondary_value <- as.character(df[[second]][i]) #finds the second value for that row

    secondary_value_split <- unlist(strsplit(secondary_value, ".", fixed = T)) #seperates the second row by '.'
    ...
    if(row_error_count > 0){ #sets instructions if errors are found
