...
#' 51.5), c(2.9, 5), c(0, 1), Stop_If_Missing = FALSE)
#'
...
#' 51.5), c(2.9, 5), c(0, 1), Stop_If_Missing = FALSE, Assume_Zero_Alcohol =
#' TRUE)
#'
...
ENERCKj_calculator <- function(Protein = "PROCNTg",
                                 Fat = "FAT_g_combined",
                                 Carbohydrate = "CHOAVLg",
                                 Fibre = "FIBTGg_combined",
                                 Alcohol = "ALCg",
  ...
  if(!missing(df)){
    ...
    df$ENERCKj_calculated <- df[[Protein]]*17 + df[[Fat]]*37 + df[[Carbohydrate]]*17 + df[[Fibre]]*8 + df[[Alcohol]]*29

    # adding temp comment column
    df$TEMPnewcomment <- paste0(df$TEMPnewcomment, "; ENERCKj_calculated calculated to be ", df$ENERCKj_calculated, " through ", Protein, "*17 + ", Fat, "*37 + ", Carbohydrate, "*17 + ", Fibre, "*8 + ", Alcohol, "*29")
    ...
    ENERCKj_calculated <- Protein*17 + Fat*37 + Carbohydrate*17 + Fibre*8 + Alcohol*29
    return(ENERCKj_calculated)
  }
}
